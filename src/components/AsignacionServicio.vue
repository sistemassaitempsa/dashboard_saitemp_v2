        ...
        <div class="container">
                ...
                <div class="col" v-for="item in radio_seleccion" :key="item.id">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault"
                            :checked="item.checked" @change="checkProfesional(item, $event)">
            ...
            <div class="row">
                <div class="col" v-for="item in radio_contratacion" :key="item.id">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault"
                            :checked="item.checked" @change="checkFacilitador(item, $event)">
                        <label class="form-check-label" for="flexSwitchCheckDefault">{{ item.nombre }}</label>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-xs-12 col-md-4">
                    <SearchList nombreCampo="Selección" :valida_campo="false" nombreItem="nombres"
                        eventoCampo="getProfesionalsector" :consulta="consulta_codigo_ciiu" :registros="profesionales"
                        @getProfesionalsector="getProfesionalsector" :ordenCampo="1"
                        placeholder="Seleccione una opción" />
                </div>
                <div class="col-xs-12 col-md-4">
                    <SearchList nombreCampo="Contratación" :valida_campo="false" nombreItem="nombres"
                        eventoCampo="getFacilitadores" :consulta="consulta_codigo_ciiu" :registros="facilitadores"
                        @getFacilitadores="getFacilitadores" :ordenCampo="1" placeholder="Seleccione una opción" />
                </div>
                <div class="col-xs-12 col-md-4  mt-3">
                    <button type="button" id="newNews" class="btn btn-success" @click="asignarUsuario()"
                        style="margin-top: 33px"> Guardar registro
                    </button>
                </div>
            </div>
            <div class="row">
            ...
            ">
                        <button type="button" style="margin: 10px 10px 5px 10px" id="btnMenu" class="btn btn-sm"
                            data-bs-toggle="button" v-for="(item, index) in asignar_facilitadores" :key="index">
                            {{ item.nombres }} {{ item.apellidos }}
                            <i class="bi bi-x" @click="asignar_facilitadores.splice(index, 1)"></i>
                        </button>
                    </div>
                </div>
            </div>
...
let radio_seleccion = ref([{ id: 1, nombre: 'seleccion_automatico', checked: false, tipo: 0 }, { id: 2, nombre: 'Asignación manual profesional', checked: false, tipo: 0 }]);
let radio_contratacion = ref([{ id: 1, nombre: 'contratacion_automatico', checked: false, tipo: 1 }, { id: 2, nombre: 'Asignación manual facilitador', checked: false, tipo: 1 }]);
...
function asignarUsuario() {
    ...
    axios.post(URL_API.value + 'api/v1/usuariodisponibleservicio', array, configHeader()).then(function (result) {
