<template>
  <div>
    <div class="container">
      ...
      <div
        class="col-8 card"
        v-if="ordenes_trabajo_empleados.length > 0 && !formulario"
      >
        <!-- <h5>{{ accion }}</h5> -->
        <form>
          ...
          <div class="row" id="contenedor-select">
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                <input
                  ...
                  @click="hover1 = !hover1"
                  autocomplete="off"
                  @input="filterResults(categoria, categorias, 'categorias')"
                  class="form-control"
            ...
            <div
              v-if="hover1 && categorias.length > 0"
              id="select1"
              @mouseleave="hover1 = false"
            >
              <div
                id="lista1"
                v-for="(item, index) in categoriasFilter"
                :key="index"
                @click="
          ...
          <div class="row" id="contenedor-select">
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                <input
                  ...
                  @click="hover2 = !hover2"
                  autocomplete="off"
                  @input="filterResults(item, items, 'items')"
                  class="form-control"
                  id="exampleInputEmail2"
                  placeholder="Cantidad"
                  aria-describedby="emailHelp"
            ...
            <div
              v-if="hover2 && items.length > 0"
              id="select2"
              @mouseleave="hover2 = false"
            >
              <div
                id="lista2"
                v-for="(descripcionItem, index) in itemsFlter"
                :key="index"
                @click="
                  addItem(descripcionItem),
                    (hover2 = !hover2),
                    (item = ''),
            ...
            >
              ...
              <div class="col-4">
                <div class="col-xs-12 col-md-12">
                  <div class="mb-3">
                    <input
                      ...
                      @keyup="
                        itemsCompletos[index].cantidad = item.cantidadItem
                      "
                    />
                  </div>
                ...
                <div class="col-2">
                  <button
                    type="button"
                    class="btn mt-2 btn-danger btn-sm"
          ...
          <div class="col">
            <div class="mb-3">
              ...
              <input
                ...
                aria-describedby="emailHelp"
                v-model="item.orden_trabajo_asignado"
                disabled
              />
            </div>
          </div>
          <div class="col">
            <div class="mb-3">
              ...
              <input
                ...
                id="exampleInputEmail1"
                aria-describedby="emailHelp"
                v-model="item.empleado_asignado"
                disabled
              />
            </div>
          </div>
            ...
            <div class="mb-3">
              ...
              <div class="mb-3">
                <textarea
                  ...
                  rows="3"
                  v-model="item.descripcion_asignado"
                ></textarea>
              </div>
            </div>
          </div>
      ...
      <div v-else>
        ...
        <div
          v-if="ordenes_trabajo_empleados.length > 0"
          class="row"
          style="clear: both"
        >
          ...
          <div class="col-xs-3 col-md-3">
            <button
              ...
              style="float: left; margin-top: 30px; margin-bottom: 30px"
              class="btn btn-warning btn-sm"
            >
              {{
                formulario == false
                  ? "Ejecutar ordenes de trabajo"
                  : "Ver mis ordenes de trabajo"
              }}
            </button>
          </div>
        </div>
        <div class="table-responsive">
          ...
          >
            ...
            <tbody>
              <tr
                v-for="(item, index) in ordenes_trabajo_empleados"
                :key="item.id"
              >
          ...
          <ul class="pagination">
            <li class="page-item">
              <a
                ...
                :key="index"
                @click="
                  pagination(item.url), (currentUrl = item.url), goScroll(80, 0)
                "
                >{{
                  index == 0
                    ? "Anterior"
  ...
  methods: {
    filterResults(value, array, nombrearray) {
      ...
      if (nombrearray == "items") {
        if (search == "") {
          search = array.filter((item) =>
            item.item.toLowerCase().match(value.toLowerCase())
          );
          this.itemsFlter = search;
        }
        if (search == "") {
          search = array.filter((item) =>
            item.descripcion.toLowerCase().match(value.toLowerCase())
          );
          this.itemsFlter = search;
        }
      }
    ...
    getOrdenesTrabajo() {
      ...
      axios
        .get(
          self.URL_API + "api/v1/ordenestrabajoasignadas/" + self.cantidad,
          config
        )
        .then(function (result) {
          self.ordenes_trabajo_empleados = result.data.data;
          self.encargados =
            result.data.data[0].responsable.split("-")[1] +
            "," +
            result.data.data[0].gestor_sst.split("-")[1] +
            "," +
            result.data.data[0].autor;
    ...
    getItems() {
      ...
      let urlEndPoint = "";
      urlEndPoint =
        self.URL_API + "api/v1/costoitemcategoria/" + self.codigo_id;
      let config = this.configHeader();
    ...
    sort(item, numero) {
      this.tabla.forEach(function (item, index) {
        document.styleSheets[0].addRule(
          ".table-responsive tr th:nth-child(" + index + "):after",
          'content: " ";'
        );
      });
      if (item.orden == "DESC") {
        document.styleSheets[0].addRule(
          ".table-responsive tr th:nth-child(" + numero + "):after",
          'content: " ↓";'
        );
    ...
    agregar() {
      let self = this;
      this.asignados.push({
        orden_trabajo_asignado: self.orden_trabajo,
        empleado_asignado: self.empleado,
        descripcion_asignado: self.descripcion,
      });
    },
    eliminar(index) {
