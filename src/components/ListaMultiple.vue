<template>
  ...
  >
    <button
      style="margin: 5px"
      class="btn btn-sm"
      v-for="(item, index) in elementos2"
      :key="index"
      type="button"
    >
      {{ item.nombre
      }}<i @click="eliminarElemento(index), swict()" class="bi bi-x"></i>
    </button>
    <div
      v-if="hover2 && elementoslist2.length > 0"
      id="select2"
      @mouseleave="hover2 = false"
    >
      <div
        id="lista2"
        v-for="(item, index) in elementoslist2"
        :key="index"
