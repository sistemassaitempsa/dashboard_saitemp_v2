  ...
  <div>
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-xs-12 col-md-3">
          <label style="width: 250px; margin-top: 25px; text-align: left"
            >Permisos</label
          >
          <select
            id="inputState1"
            ...
            v-model="permiso_select"
            @change="permisoId(permiso_select)"
          >
            <option
              ref="item_select"
              v-for="(item, index) in permisos_usuario"
              :key="index"
            >
              {{ item.nombre }}
            </option>
        ...
        <div class="mb-3" v-if="asignar_permisos.length > 0">
          ...
          >
            <button
              ...
              class="btn btn-sm"
              data-bs-toggle="button"
              v-for="(item, index) in asignar_permisos"
              :key="index"
            >
              {{ item.nombre }}
              <i class="bi bi-x" @click="asignar_permisos.splice(index, 1)"></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <Tabla
        ...
        :campos="campos"
        :eliminar="permisos[19].autorizado"
        @response="response"
        @clear="clear"
        @check="check"
...
export default {
  ...
  methods: {
    getUsuarios(item, index = null) {
      ...
      if (item != null && index == null) {
        ...
        if (!this.actualizar_menu) {
          const usuario_existe = this.asignar_usuarios.some(
    ...
    FiltroPorUsuario(id) {
      ...
      axios
        .get(
          self.URL_API + "api/v1/filtroporusuario/" + id + "/" + self.cantidad,
          config
        )
        .then(function (result) {
          self.datos = result;
    ...
    getPermisos_usuario() {
      ...
      let config = this.configHeader();
      axios
        .get(self.URL_API + "api/v1/permisoslista", config)
        .then(function (result) {
    ...
    asignarPermiso() {
      ...
      if (this.checks.length > 0) {
        ...
        let config = this.configHeader();
        axios.post(urlEndPoint, permiso_rol, config).then(function (result) {
          self.loading = false;
          self.showAlert(result.data.message, result.data.status);
          self.getItems();
          self.clear();
      ...
      this.permisos_usuario.forEach(function (element) {
        if (permiso == element.nombre) {
          ...
          if (!self.actualizar_menu) {
            const permiso_existe = self.asignar_permisos.some(
              (permiso) => permiso.id === element.id
            );
            if (!permiso_existe) {
              self.asignar_permisos.push({
                id: element.id,
