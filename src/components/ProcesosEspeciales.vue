<template>
  <div class="container">
    ...
    <div class="row">
      <div class="col-6">
        <SearchList
          nombreCampo="Procesos"
          @getProcedureId="getProcedureId"
          eventoCampo="getProcedureId"
          nombreItem="nom_proc"
          :registros="procedures"
    ...
    <div class="row" v-for="(item, index) in form_procedures" :key="index">
      <div class="col-6" v-if="item.tip_obj.trim() == 'LA'">
        <SearchTable
          :nombreCampo="item.des_param"
          @getValue="getValue"
          @getGeneric="getGeneric(item.tab_hlp, item.nom_hlp, item.des_hlp)"
          eventoCampo="getGeneric"
          :nombreItem="item.nom_hlp"
          :endpoint="endpoint"
          :item="item.tab_hlp"
          :item1="item.nom_hlp"
          :item2="item.des_hlp"
          :posicion="item.ord_cmp"
      ...
      <div class="col-6" v-if="item.tip_obj.trim() == 'L'">
        <div>
          ...
          <select
            class="form-select"
            v-model="text_field[item.ord_cmp - 1]"
            aria-label="Default select example"
          >
            <option
              v-for="(item, index) in item.des_hlp.split(',')"
              :key="index"
            >
              {{ item }}
            </option>
    ...
    <div class="row">
      <div class="col">
        <button
          v-if="procedures.length > 0 && show_button_process"
          class="btn btn-success"
          @click="excecuteProcedure(), (show_table = true)"
        >
          Procesar
        </button>
      </div>
      <div v-if="btnexport" class="col">
        <button
          ...
          @click="
            showAlert(
              'Por favor espere un momento mientras el archivo se procesa y se descarga.',
              'success'
            )
          "
          type="button"
        ...
        >
          <a
            :href="
              URL_API + 'api/v1/procesosespecialesexport/' + base64consulta
            "
  ...
  methods: {
    ...
    getGeneric(item, item1, item2) {
      ...
      axios
        .get(
          self.URL_API +
            "api/v1/listasprocesosespeciales/" +
            item +
            "/" +
            item1 +
            "/" +
    ...
    excecuteProcedure() {
      ...
      if (this.text_field[this.text_field.length - 1] == this.name_procedure) {
        this.text_field.splice(
          this.text_field.length - 1,
          1,
          this.name_procedure
        );
      ...
      axios
        .get(
          self.URL_API + "api/v1/ejecutaprocesosespeciales",
          { params: { parametros } },
          config
        )
        .then(function (result) {
